...
import EthWalletAddress from "./EthWalletAddress";

const EthPopup = ({ isOpen, setIsOpen, depositAmt }: any) => {
  const copyEthToClipboard = async (text: string) => {
    try {
        ...
        >
          <Dialog.Panel className="px-8 pt-1 pb-6 space-y-12 align-middle shadow-xl transition-all max-w-2xl transform z-50 bg-slate-900 m-auto -mt-[680px] rounded-2xl lg:-mt-[800px]">
            ...
            <section className="w-full">
              <EthWalletAddress copyToClipboard={copyEthToClipboard} depositAmt={depositAmt} />
            </section>
          </Dialog.Panel>
        </Transition.Child>
      </Dialog>
