...
const MainBody = () => {
  ...
  return (
      <section className="space-y-12 px-8 pt-6 w-full">
          <section>
              <h1 className="text-2xl text-white font-bold">Good {timeOfDay}, {user.name}</h1>
          </section>
          <section className="flex h-max w-full justify-between lg:block lg:space-y-8">
            <section className="bg-slate-900 pt-20 space-y-12 w-[60%] shadow-sm rounded-xl lg:w-full">
              <span className="space-y-4">
                <p className="text-xs font-medium text-neutral-400 text-center">PORTFOLIO BALANCE</p>
                <p className="text-4xl text-center font-bold text-white">{formattedCurr(user.balance.totalBalance)}</p>
              </span>
              <section className="bg-slate-900 rounded-b-xl -mt-4">
