    ...
    return (    
        <div className="flex items-center justify-between bg-transparent px-8">
            <span className="relative lg:hidden">
                <input type="search" className="placeholder:pl-12 placeholder:font-normal border border-color-slate-700 h-10 px-12 bg-transparent rounded-full" value={searchValue} onChange={(e) => (setSearchValue(e.target.value))} placeholder="search" />
            ...
            <span className="">
                <Popover className="relative" data-popover-target="money-dropdown">
                    <Popover.Button className="flex items-center bg-white rounded-full">
                        <span className="space-x-2 flex items-center  px-4 py-2">
                            <p className="font-medium text-center m-auto text-zinc-900">Move Money</p>
                            <ChevronDownIcon className="w-4 h-4 text-slate-900" />
                        </span>
                    </Popover.Button>
                    <Transition
                        as={Fragment}
                        ...
                        leaveTo="opacity-0 translate-y-1">
                            <Popover.Panel id="money-dropdown" className="absolute top-8 left-0 w-64 mt-2 bg-slate-900 rounded-lg shadow-xl">
                                <MoveMoneyDropdown />
                            </Popover.Panel>
                        </Transition>
                </Popover>
            ...
            <Popover className="">
                <Popover.Button>
                    <span className="rounded-full bg-slate-900 w-10 h-10 relative items-center flex">
                ...
                <Transition
                    ...
                    leaveFrom="opacity-100 translate-y-0"
                    leaveTo="opacity-0 translate-y-1">
                        <Popover.Panel className="absolute top-0 right-0 w-fit bg-slate-900 shadow-xl z-50">
                           <NotificationPanel />
                        </Popover.Panel>
                    </Transition>
            </Popover>
